...
public class GameDisplayActivity extends Activity {
	...
	public void updateBoardUI() {
		//for (int i = 0; i <= 8; i++) {
			...
			// currentImageButton.setClickable(false); //disables the ImageButton if it contains text
		//} //end for
	...
	OnClickListener gameImageButtonListener = new OnClickListener() {
		public void onClick(View v) {
			...
			// this for loop matches the current ImageButton id to the id stored in the ImageButtonLocation array 
			for (int i = 0; i <= 8; i++) {
				String clickedId = v.toString();
				if (clickedId.equals(ImageButtonLocation[i])) location = i;
			}
			
			if (lastBtnPress >= 0) boardUI[lastBtnPress].setImageResource(R.drawable.transparent);
			
			ImageButton currentImageButton = boardUI[location];
			int bckgrnd = R.drawable.transparent;
			if (moveMarker == 1) bckgrnd = R.drawable.xmark;
			else if (moveMarker == 2) bckgrnd = R.drawable.omark;
			currentImageButton.setImageResource(bckgrnd);
	...
	public void updateStatusMessage() {
		...
		if (winner == 0) {
			...
			viewWhichTurn.setText(message);
			for (int i = 0; i < 9; i++) {
				if (boardUI[i].isClickable()) boardUI[i].setClickable(false);
			} //end for 
    ...
    OnClickListener clickSubmitButtonListener = new OnClickListener() {
    	public void onClick(View v) {
    		...
    		if (lastBtnPress == -1) finish();    			
    			...
    			if (winner == 0) moveMarker = manager.getCurrentPlayer();
